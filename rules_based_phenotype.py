...
from pyspark.sql.functions import col, coalesce, to_date, concat, \
    regexp_replace, lit, when, lower, upper, trim
import re
...
opioid_phenotyping_omop = {"care_site": "/projects/cch/opioid-phenotyping/data/raw/omop/care_site",
...
def read_in_targets():
    ...
    df_kevin.loc[:,list(set(df_kevin.columns.tolist()) - set(['visit_occurrence_id']))] = df_kevin[list(set(df_kevin.columns.tolist()) - set(['visit_occurrence_id']))] \
...
def get_tables():
    ...
    medications_classification = spark.read.parquet('{}medication_classification'.format(base_data_loc))
    ...
    BirthYearDF = VO.where(
        col('visit_occurrence_id').isin(VO_ids)) \
        .select('visit_occurrence_id', 'person_id', 'visit_end_datetime') \
        .join(PER, 'person_id').select('visit_occurrence_id', f.year(PER.birth_datetime) \
        .alias('BirthYear'), 'visit_end_datetime')

    #get prior visit occurrences
    VO_ids_w_prior = VO.where(
        col('visit_occurrence_id').isin(VO_ids)) \
        .select('person_id', 'visit_end_datetime').alias('a') \
        .join(VO.alias('b'), 'person_id') \
        .filter((col('a.person_id') == col('b.person_id')) & (col('b.visit_end_datetime') <= col('a.visit_end_datetime'))) \
        .select('b.visit_occurrence_id').toPandas()['visit_occurrence_id'].tolist()
    # Get cheif Complaints
    CC = diagnosis_extract \
        .where(col('visit_occurrence_id').isin(VO_ids)) \
    ...
    CC = CC.select([c if c == 'visit_occurrence_id' else col(c).alias('CCTBL_'+c) for c in CC.columns])
    # Maybe refactor to function?
    # Lets try to get all levels of icd codes there are a total of 6 characters for icd10
    DE_lst = [None]*8
    for i in [1,2,3,4,5,6,7,8]:
        DE_tmp = diagnosis_extract \
            .where(col('visit_occurrence_id').isin(VO_ids)) \
            .withColumn("condition_source_value", \
                        when(col("condition_source_value").rlike('[A-TV-Z][0-9][0-9AB]\.?[0-9A-TV-Z]{0,4}'),substring(upper(col("condition_source_value")),1,i)) \
            ...
            .pivot("condition_source_value").sum('count').fillna(0)
    ...
    DE = DE.select([c if c == 'visit_occurrence_id' else col(c).alias('DETBL_'+ c) for c in DE.columns])
    ...
    for ATC_LEVEL in [1,2,3,4,5]:
        MED_tmp = medications.where(col('visit_occurrence_id').isin(VO_ids)).join(
        medications_classification, 'drug_concept_id').select(
        'visit_occurrence_id', "ATC_{}".format(ATC_LEVEL)).withColumn("count", lit(1)).groupby(col('visit_occurrence_id')).pivot(
            "ATC_{}".format(ATC_LEVEL)).sum('count').fillna(0)
        #this dot (.) is problematic, cant call without backticks replacing with underscore
    ...
    UT = urine_toxicology.where((col('visit_occurrence_id').isin(VO_ids)) & (col('isSubstancePositive') == 'True')) \
    ...
    BC = blood_culture.where((col('visit_occurrence_id').isin(VO_ids)) & (col('no_growth_in_specimen') == 'False')) \
    ...
    ACH = alcohol \
        .where((col('visit_occurrence_id').isin(VO_ids)) & (col('alcohol_positive') == 'True')) \
        .select('visit_occurrence_id', 'alcohol_positive') \
        .withColumn("count", lit(1)) \
        .groupby('visit_occurrence_id') \
        .sum('count').alias('alcohol_positive') \
        .fillna(0) \
    ...
    MED_new = MED.select([c if c == 'visit_occurrence_id' else col(c).alias('MEDTBL_'+c) for c in MED.columns])
    ME_new = ME.select([c if c == 'visit_occurrence_id' else col(c).alias('METBL_'+c) for c in ME.columns])
    UT_new = UT.select([c if c == 'visit_occurrence_id' else col(c).alias('UTTBL_'+ c) for c in UT.columns])
    BC_new = BC.select([c if c == 'visit_occurrence_id' else col(c).alias('BCTBL_'+ c) for c in BC.columns])
    ACH_new = ACH.select([c if c == 'visit_occurrence_id' else col(c).alias('ACHTBL_'+ c) for c in ACH.columns])

    #.join(ME_new, 'visit_occurrence_id', 'left') \ #Remove vitals/measurements
    res = DE_new.join(MED_new, 'visit_occurrence_id', 'left') \
...
def get_best_features(num_feats):
    ...
    for target in list(set(df_kevin.columns.tolist()) - set(['visit_occurrence_id'])):
        print(target)
        pdf = df_kevin[['visit_occurrence_id', target]].join(df.set_index('visit_occurrence_id'), on='visit_occurrence_id').fillna(0)
        ...
        from sklearn.feature_selection import RFE
        from sklearn.linear_model import LogisticRegression
        rfe_selector = RFE(estimator=LogisticRegression(penalty='l1', C=0.3, solver='liblinear'), n_features_to_select=num_feats, step=10, verbose=5)
        ...
        from sklearn.linear_model import LogisticRegression
        embeded_lr_selector = SelectFromModel(LogisticRegression(penalty="l1", C=0.2, solver='liblinear'), max_features=num_feats)
        ...
        embeded_rf_selector = SelectFromModel(RandomForestClassifier(n_estimators=300), max_features=num_feats)
        ...
        feature_selection_df = pd.DataFrame({'Feature': feature_name,  'xChi-2':chi_support, 'RFE':rfe_support, 'Logistics':embeded_lr_support,'Random Forest':embeded_rf_support, 'GradientBoosting':embeded_xgb_support, 'shap_imp': shap_support})
        ...
        # Saving results
        feature_selection_df.to_csv('feature_selection_results/feature_list_{}_target_{}_ICD_{}_ATC_{}.csv'.format(num_feats, target, ICD_LEVEL, ATC_LEVEL))
...
def read_best_features(num_feats):
    ...
    for target in targets:
        feat_df = pd.read_csv('feature_selection_results/feature_list_{}_target_{}_ICD_{}_ATC_{}.csv'.format(num_feats, target, ICD_LEVEL, ATC_LEVEL), index_col=0)
        feature_list_dict[target] = feat_df[~feat_df.Feature.str.contains("person_id")][:top_n]['Feature'].to_list()
    ...
    for idx, feat in enumerate(feats_copy):
        if 'DETBL_' in feat:
            ...
            if atc_code in ATC['ATC_code'].values:
                feats[idx] = 'MEDTBL_' + ATC[ATC['ATC_code'] == atc_code]['Preferred Label'].values[0]
    return feats

def plot_calibration_curve(est, name_, fig_index, X_train, X_val, X_test, y_train, y_val, y_test, cv, target):
    ...
    plt.tight_layout()
    plt.savefig('feature_selection_results/Calibration_plot_{}_target_{}_ICD_{}_ATC_{}.png' \
                .format(num_feats, target, ICD_LEVEL, ATC_LEVEL), dpi=300)
    plt.show()
...
def plot_PR_curve(y_test, probs, target):
    ...
    plt.title('PR_Curve_AUC_{}_target_{}_ICD_{}_ATC_{}' \
                .format(num_feats, target, ICD_LEVEL, ATC_LEVEL))
    ...
    plt.savefig('feature_selection_results/PR_Curve_AUC_{}_target_{}_ICD_{}_ATC_{}.png' \
                .format(num_feats, target, ICD_LEVEL, ATC_LEVEL), dpi=300)
    plt.show()
...
def plot_ROC_curve(y_test, probs, target):
    ...
    # method I: plt
    import matplotlib.pyplot as plt
    plt.title('ROC_Curve_AUC_{}_target_{}_ICD_{}_ATC_{}' \
                .format(num_feats, target, ICD_LEVEL, ATC_LEVEL))
    ...
    plt.savefig('feature_selection_results/ROC_Curve_AUC_{}_target_{}_ICD_{}_ATC_{}.png' \
                .format(num_feats, target, ICD_LEVEL, ATC_LEVEL), dpi=300)
    plt.show()
def train_model():
    models = {}
    ...
    SEED = 42
    param_grid = {'max_depth': np.arange(2, 6), 'max_features': np.arange(50, 151), 
                  'min_samples_leaf': np.arange(11, 16)}#, 'min_samples_split': np.arange(20, 30)}
    ...
    for target, feats in feature_list_dict.items():
        ...
        with open('feature_selection_results/CR_{}_target_{}_ICD_{}_ATC_{}.json' \
                    .format(num_feats, target, ICD_LEVEL, ATC_LEVEL), 'w') as fp:
            json.dump(CR_dict, fp)
        # Plots
        plot_ROC_curve(y_test, scores, target)
        plot_PR_curve(y_test, scores, target)
        plot_calibration_curve(tree_clf.best_estimator_, 'Best Tree', 1, X_train, X_val, X_test, y_train, y_val, y_test, 'prefit', target)
        plt.figure(figsize=(24,12))
        plt.rcParams['figure.dpi'] = 150
        plot_tree(tree_clf.best_estimator_, feature_names=human_read_feats(feats), fontsize=5, filled=False, rounded=True)#left is true
        plt.savefig('feature_selection_results/tree_{}_target_{}_ICD_{}_ATC_{}.png' \
                    .format(num_feats, target, ICD_LEVEL, ATC_LEVEL), dpi=150)
        plt.show()
    return models
    
if __name__== "__main__":
    # Set PYSPARK_PYTHON
    os.environ["PYSPARK_PYTHON"] = "./temp_envs/py3env/bin/python"# this has to be here so we can execute a udf

    # Zip Modules
    os.system("zip modules.zip -r modules/*.py")
    spark = SparkSession\
        .builder\
        .appName("OpioidPhenotyping")\
        .enableHiveSupport()\
        .getOrCreate()
    ...
    # Binarize columns
    colsToBinarize = list(set([c for c in df.columns.tolist() if 'DETBL_' in c] \
    ...
    ICDtoDrop = [col for col, val in df[[x for x in df.columns if 'DETBL_' in x]].sum().iteritems() if val < 100]
